...
import { 
  ...
  type MLScore,
  type CompositeScore
} from '@/lib/compositeScoring';

interface NetworkAnalysisData {
  suspicious_networks: any[];
  fraud_indicators: {
    high_fraud_score_users: any[];
    rapid_claim_filers: any[];
  };
  risk_scores: { [userId: string]: any };
...
const ClaimDetailsModal: React.FC<{ claim: any; onClose: () => void }> = ({ claim, onClose }) => {
  ...
  const [networkAnalysis, setNetworkAnalysis] = useState<NetworkAnalysisData | null>(null);
  ...
  useEffect(() => {
    if (claim) {
      const incidentPhotos = claim.incident_photos || [];
      const supportingDocs = claim.supporting_documents || [];
      setClaimImages({ incident_photos: incidentPhotos, supporting_documents: supportingDocs });
      
      // Load existing vision analysis from Firebase if available
      if (claim.vision_analysis) {
        console.log('📥 Loading existing vision analysis from Firebase');
        setVisionAnalysisResults({
          ocrResults: claim.vision_analysis.ocrResults || {},
          cnnResults: claim.vision_analysis.cnnResults || {},
          isAnalyzing: {}
        });
      }
    }
  ...
  // Fetch network analysis data for this user
  const fetchNetworkAnalysis = async () => {
    console.log('🔍 Starting network analysis for user:', claim?.userId);
    console.log('📊 Claim data keys:', Object.keys(claim || {}));
    console.log('📊 Claim data:', claim);
    
    if (!claim?.userId) {
      console.error('❌ No userId found in claim data');
      return;
    }
    ...
    try {
      ...
      if (cacheData && cacheData.timestamp) {
        ...
        if (cacheAge < maxCacheAge && cacheData.networkAnalysis) {
          ...
          setNetworkAnalysis(cacheData.networkAnalysis);
      ...
      // First, try to fetch all data like the main Network Analysis page does
      ...
      console.log('📤 Sending request to API with full data:', requestData);
      ...
      while (retries > 0) {
        try {
          ...
          if (response.ok) {
            ...
            if (result.success) {
              console.log('✅ Network analysis successful, setting data:', result.results);
              setNetworkAnalysis(result.results);
              ...
              // Recalculate composite score with network analysis
              await recalculateCompositeScore();
              
              return; // Success, exit retry loop
            } else {
              console.error('❌ API returned success: false, error:', result.error);
              if (result.error.includes('Existing exports of data')) {
          ...
          throw fetchError; // Re-throw if no retries left
        }
      }
  ...
  const handleGeminiAnalysis = async (forceRegenerate = false) => {
    if (!claim || geminiProcessing) return;
    
    setGeminiProcessing(true);
    try {
      console.log(`🤖 Starting Gemini analysis for claim: ${claim.id}${forceRegenerate ? ' (FORCE REGENERATE)' : ''}`);
      
      // If forcing regeneration, temporarily clear the existing analysis
      const claimData = forceRegenerate ? { ...claim, gemini_analysis: '' } : claim;
      
      // Filter network analysis to only include networks where the current user is involved
      const filteredNetworkAnalysis = networkAnalysis ? {
        ...networkAnalysis,
        suspicious_networks: networkAnalysis.suspicious_networks?.filter((network: any) => {
          const users = network.users || [];
          const userNames = network.user_names || [];
          return users.some((user: string) => user === claim.userId) || 
                 userNames.some((user: string) => user === claim.userId);
        }) || []
      } : null;
      
      console.log('🔍 Filtered network analysis for Gemini:', {
        originalNetworks: networkAnalysis?.suspicious_networks?.length || 0,
        filteredNetworks: filteredNetworkAnalysis?.suspicious_networks?.length || 0,
        currentUserId: claim.userId
      });
      
      const geminiResult = await processGeminiAnalysis(claimData, filteredNetworkAnalysis, visionAnalysisResults);
  ...
  const handleOCRAnalysis = async (document: any, index: number) => {
    ...
    try {
      ...
      setVisionAnalysisResults(prev => {
        ...
        // Store OCR results in Firebase and recalculate composite score
        updateVisionAnalysisInFirebase(newResults);
  ...
  const handleCNNAnalysis = async (photo: any, index: number) => {
    ...
    try {
      ...
      setVisionAnalysisResults(prev => {
        ...
        // Store CNN results in Firebase and recalculate composite score
        updateVisionAnalysisInFirebase(newResults);
  ...
  // Update vision analysis in Firebase and recalculate composite score
  const updateVisionAnalysisInFirebase = async (visionResults: any) => {
    try {
      console.log('💾 Storing vision analysis results in Firebase...');
      
      // Store vision analysis results
      await updateDoc(doc(db, 'claims', claim.id), {
        vision_analysis: {
          ocrResults: visionResults.ocrResults || {},
          cnnResults: visionResults.cnnResults || {},
          last_updated: new Date().toISOString()
        }
      });
      
      console.log('✅ Vision analysis stored, recalculating composite score...');
      
      // Recalculate composite score
      await recalculateCompositeScore(visionResults);
  ...
  // Recalculate composite fraud score
  const recalculateCompositeScore = async (visionResults?: any) => {
    try {
      console.log('🔄 Recalculating composite fraud score...');
      
      // Get ML score
      const mlScore: MLScore | null = claim.fraudScore ? {
      ...
      // Get network score
      const networkScore = networkAnalysis ? calculateNetworkScore(claim.userId, networkAnalysis) : null;
      
      // Get OCR score with user and claim details
      const resultsToUse = visionResults || visionAnalysisResults;
      let ocrScore = null;
      if (Object.keys(resultsToUse.ocrResults || {}).length > 0) {
        // Fetch user details
        ...
        ocrScore = calculateOCRScore(resultsToUse.ocrResults, userDetails, claim);
      }
      
      // Get CNN score with incident description and claim details
      let cnnScore = null;
      if (Object.keys(resultsToUse.cnnResults || {}).length > 0) {
        const incidentDescription = claim.description || claim.incident_description || '';
        cnnScore = calculateCNNScore(resultsToUse.cnnResults, incidentDescription, claim);
      }
      
      // Calculate composite score
      const compositeScore = calculateCompositeScore(mlScore, networkScore, ocrScore, cnnScore);
      
      console.log('📊 Composite score calculated:', compositeScore);
      ...
      console.log('✅ Composite score stored in Firebase');
      
    } catch (error) {
      console.error('❌ Failed to recalculate composite score:', error);
    }
  };
  ...
  // Add debugging for ML analysis data
  console.log('🔍 CLAIM MODAL DEBUG for claim:', claim.id, {
    fraudScore: claim.fraudScore,
    fraud_score: claim.fraud_score,
    riskLevel: claim.riskLevel,
    fraudExplanation: claim.fraudExplanation,
    detailed_explanation: claim.detailed_explanation,
    mlAnalysisDate: claim.mlAnalysisDate,
    isMLAnalyzed: !!((claim.fraudScore || claim.fraud_score) && (claim.fraudScore || claim.fraud_score) > 0),
    isMLAnalyzedAlt: !!(claim.fraud_score && claim.fraud_score > 0),
    claimKeys: Object.keys(claim).filter(key => key.includes('fraud') || key.includes('ml') || key.includes('risk'))
  });
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
              ...
              <div className="space-y-2 text-sm text-slate-300">
                ...
                {claim.description && (
                  <div className="mt-3">
                    ...
                    <p className="text-slate-400 mt-1 bg-slate-700/50 p-3 rounded-lg">
                      {claim.description}
                    </p>
                  </div>
                )}
          ...
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-xl border-2 border-slate-700 mb-6">
            <h4 className="text-white font-bold text-xl mb-4 flex items-center gap-2">
              <Brain className="w-6 h-6 text-purple-400" />
              Overall Fraud Score Analysis
            </h4>
            ...
            {claim.composite_fraud_score ? (
              <div className="space-y-6">
                ...
                <div className="text-center bg-slate-800/50 p-6 rounded-lg border border-slate-600">
                  ...
                  <div className={`text-2xl font-semibold mb-2 ${
                    claim.composite_risk_level === 'high' ? 'text-red-400' :
                    claim.composite_risk_level === 'medium' ? 'text-yellow-400' :
                    'text-green-400'
                  }`}>
                    {claim.composite_risk_level?.toUpperCase()} RISK
                  </div>
                  <div className="text-sm text-slate-400">
                    {claim.composite_confidence}% Analysis Confidence
                  </div>
                  <div className="text-xs text-slate-500 mt-2">
                    Components: {claim.composite_score?.components_analyzed?.join(', ') || 'N/A'}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-600">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-slate-300 font-semibold">ML Model Analysis</span>
                      <span className="text-white font-mono text-lg">{(claim.composite_score?.ml_score * 100).toFixed(1)}%</span>
                    </div>
                    <div className="w-full bg-slate-700 rounded-full h-2">
                      <div 
                        className="bg-blue-500 h-2 rounded-full transition-all" 
                        style={{ width: `${claim.composite_score?.ml_score * 100}%` }}
                      />
                    </div>
                    <div className="text-xs text-slate-400 mt-1">Weight: {(claim.composite_score?.weights?.ml * 100).toFixed(0)}%</div>
                  </div>

                  <div className="bg-slate-800/50 p-4 rounded-lg border border-purple-500/30">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-purple-300 font-semibold">Network Analysis</span>
                      <span className="text-white font-mono text-lg">{(claim.composite_score?.network_score * 100).toFixed(1)}%</span>
                    </div>
                    <div className="w-full bg-slate-700 rounded-full h-2">
                      <div 
                        className="bg-purple-500 h-2 rounded-full transition-all" 
                        style={{ width: `${claim.composite_score?.network_score * 100}%` }}
                      />
                    </div>
                    <div className="text-xs text-slate-400 mt-1">Weight: {(claim.composite_score?.weights?.network * 100).toFixed(0)}%</div>
                  </div>

                  <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-600">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-slate-300 font-semibold">OCR Document Analysis</span>
                      <span className="text-white font-mono text-lg">{(claim.composite_score?.ocr_score * 100).toFixed(1)}%</span>
                    </div>
                    <div className="w-full bg-slate-700 rounded-full h-2">
                      <div 
                        className="bg-green-500 h-2 rounded-full transition-all" 
                        style={{ width: `${claim.composite_score?.ocr_score * 100}%` }}
                      />
                    </div>
                    <div className="text-xs text-slate-400 mt-1">Weight: {(claim.composite_score?.weights?.ocr * 100).toFixed(0)}%</div>
                  </div>

                  <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-600">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-slate-300 font-semibold">CNN Image Analysis</span>
                      <span className="text-white font-mono text-lg">{(claim.composite_score?.cnn_score * 100).toFixed(1)}%</span>
                    </div>
                    <div className="w-full bg-slate-700 rounded-full h-2">
                      <div 
                        className="bg-orange-500 h-2 rounded-full transition-all" 
                        style={{ width: `${claim.composite_score?.cnn_score * 100}%` }}
                      />
                    </div>
                    <div className="text-xs text-slate-400 mt-1">Weight: {(claim.composite_score?.weights?.cnn * 100).toFixed(0)}%</div>
                  </div>
                </div>

                {/* Detailed Breakdown */}
                {claim.composite_score?.detailed_breakdown && (
                  <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-600">
                    <h5 className="text-white font-semibold mb-2">Detailed Analysis</h5>
              ...
              <div className="text-center py-8">
                <div className="text-slate-400 mb-2">Comprehensive analysis in progress...</div>
                <div className="text-sm text-slate-500">ML, Network, OCR, and CNN analyses are being performed automatically</div>
              </div>
            )}
          </div>
          ...
          <div className="bg-slate-800 p-4 rounded-lg mb-6">
            ...
            <MLExplanationDisplay 
              ...
              fraudScore={claim.fraudScore || claim.fraud_score || 0}
            ...
            {claim.fraudScore && (
              <div className="mt-4">
                <button
                  onClick={() => handleGeminiAnalysis(geminiAnalysisResult && 
          ...
          <div className="bg-slate-800 p-4 rounded-lg mb-6">
            ...
            {!networkAnalysis ? (
              <div className="text-center py-4">
                <button
                  onClick={fetchNetworkAnalysis}
                  disabled={isLoadingNetwork}
                  className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold py-2 px-4 rounded-lg transition-colors flex items-center gap-2 mx-auto"
                >
                  {isLoadingNetwork ? (
                    <>
                      <div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent"></div>
                ...
                <div className="mt-2 text-xs text-gray-400">
                  User ID: {claim?.userId || 'Not found'}
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                {/* User Risk Score */}
                {networkAnalysis.risk_scores[claim.userId] && (
                  <div className="bg-slate-700 p-3 rounded-lg">
                    <h5 className="text-white font-medium mb-2">User Risk Profile</h5>
                    <div className="flex items-center gap-4">
                      <div className="text-sm">
                        <span className="text-gray-400">Overall Risk:</span>
                        <span className={`ml-2 px-2 py-1 rounded text-xs font-semibold ${
                          networkAnalysis.risk_scores[claim.userId].overall_risk > 0.7 ? 'bg-red-500' :
                          networkAnalysis.risk_scores[claim.userId].overall_risk > 0.4 ? 'bg-yellow-500' : 'bg-green-500'
                        }`}>
                          {(networkAnalysis.risk_scores[claim.userId].overall_risk * 100).toFixed(1)}%
                        </span>
                      </div>
                      <div className="text-sm text-gray-400">
                        {networkAnalysis.risk_scores[claim.userId].claim_count} total claims
                      </div>
                    </div>
                  </div>
                )}
                ...
                {/* Rapid Filer Check */}
                {networkAnalysis.fraud_indicators.rapid_claim_filers.some((filer: any) => filer.userId === claim.userId) && (
                  <div className="bg-orange-500/20 border border-orange-500/30 p-3 rounded-lg">
                    <div className="flex items-center gap-2 text-orange-400 font-medium">
                      <AlertTriangle className="w-4 h-4" />
                      Rapid Claim Filer Detected
                    </div>
                    <p className="text-sm text-orange-300 mt-1">
                      This user has filed multiple claims in a short time period.
                    </p>
                  </div>
                )}

                {/* Suspicious Networks */}
                {networkAnalysis.suspicious_networks && networkAnalysis.suspicious_networks.length > 0 && (() => {
                  // Filter networks to only show networks where the current user is involved
                  const filteredNetworks = networkAnalysis.suspicious_networks.filter((network: any) => {
                    const users = network.users || [];
                    const userNames = network.user_names || [];
                    const isUserInNetwork = users.some((user: string) => user === claim.userId) || 
                                           userNames.some((user: string) => user === claim.userId);
                    
                    // Only show networks where the current user is involved
                    return isUserInNetwork;
                  });
                  ...
                  return filteredNetworks.length > 0 ? (
                    <div className="bg-red-500/20 border border-red-500/30 p-3 rounded-lg">
                      <h5 className="text-red-400 font-medium mb-2 flex items-center gap-2">
                        <AlertTriangle className="w-4 h-4" />
                        Suspicious Networks ({filteredNetworks.length})
                      </h5>
                          ...
                          <div key={index} className="bg-slate-700 p-2 rounded text-sm">
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <span className={`px-2 py-1 rounded text-xs font-semibold ${
                                network.risk_score > 0.7 ? 'bg-red-500' :
                                network.risk_score > 0.4 ? 'bg-yellow-500' : 'bg-green-500'
                              }`}>
                                {network.risk_score > 0.7 ? 'High' : network.risk_score > 0.4 ? 'Medium' : 'Low'}
                              </span>
                            </div>
                            <div className="text-gray-300 text-xs">
                              <div>Shared: {network.shared_attribute}</div>
                              <div>Users: {network.user_names ? network.user_names.join(', ') : network.users.join(', ')}</div>
                            </div>
                          </div>
                        ))}
                      </div>
